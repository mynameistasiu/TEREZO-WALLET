...
const CURRENT_USER_ID_KEY = "tw_current_user_id"
const scopedTxKey = (userId) => (userId ? `${TX_KEY}_${userId}` : TX_KEY)
...
export const loginUserLocal = ({ email, password }) => {
  ...
  localStorage.setItem(CURRENT_USER_ID_KEY, String(found.id))
...
export const getCurrentUser = () => {
  const direct = readJson(USER_KEY, null)
  if (direct && direct.id) return direct

  const currentId = localStorage.getItem(CURRENT_USER_ID_KEY)
  if (!currentId) return null
  const users = ensureUsersStore()
  const fallback = users.find((u) => Number(u.id) === Number(currentId))
  if (!fallback) return null
  const safeUser = { ...fallback }
  delete safeUser.password
  writeJson(USER_KEY, safeUser)
  return safeUser
}
...
export const addLocalTransaction = (tx, userId = null) => {
  const key = scopedTxKey(userId)
...
export const getLocalTransactions = (userId = null) => readJson(scopedTxKey(userId), [])
